        ...
        if($item->rowCount() > 0){
            while($row = $item->fetch(PDO::FETCH_ASSOC)){
                ...
                echo '<div class="cardi h-100">';
                    ...
                    if($_SESSION['nivel'] == 0){
                        echo '<button class="btn btn-success" disabled>Fazer Pedido</button>';
                    } 
                    else{
                        echo '<button class="btn btn-success btnC" data-toggle="modal" data-target="#pedido'.$row["idItens"].'">Fazer Pedido</button>';
                    }                      
                    echo '</div>';
                echo '</div>';
